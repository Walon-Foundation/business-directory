...
export const business = pgTable("business", {
  ...
  verificationLevel: verificationLevelEnum("verification_level").default("pending").notNull(),
  ...
  capitalInvestment: decimal("capital_investment", { precision: 15, scale: 2 }),
  ...
  recentNews: jsonb("recent_news").$type<Array<{
    title: string;
    date: string;
    source: string;
    link?: string;
  ...
  // Compliance records
  complianceRecords: jsonb("compliance_records").$type<Array<{
    type: string;
    status: string;
    dueDate?: string;
  ...
  revenueGrowth: jsonb("revenue_growth").$type<Array<{
    year: number;
    growth: number;
    amount: string;
  }>>().default([]),
  ...
  marketCoverage: jsonb("market_coverage").$type<Array<{
    region: string;
    percentage: number;
    notes?: string;
  }>>().default([]),
  ...
  esgScores: jsonb("esg_scores").$type<{
  ...
  riskAssessment: jsonb("risk_assessment").$type<{
    ...
    marketRisk: number;
    overallRisk: number;
  }>().default({ financialRisk: 0, operationalRisk: 0, complianceRisk: 0, marketRisk: 0, overallRisk: 0 }),
  
  // Audit information
  auditInfo: jsonb("audit_info").$type<{
  ...
  // Regulatory filings
  regulatoryFilings: jsonb("regulatory_filings").$type<Array<{
    type: string;
    filingDate: string;
    status: string;
    documentUrl?: string;
