...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const newComplaint = (await db.insert(complaints).values(sanitizedData).returning())[0]
...
export async function GET(req:NextRequest, { params}:{params:Promise<{id:string}>}){
    try{
        ...
        const value = await db.select().from(complaints).where(eq(complaints.businessId,id)).execute()
        if(value.length === 0){
            return NextResponse.json({
                ok:true,
                data:[],
