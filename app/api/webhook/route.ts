 ...
 */
async function sendWasenderMessage(chatId: string, message: string): Promise<any> {
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      if (error.code === 'ECONNABORTED' || error.code === 'ETIMEDOUT') {
        ...
        throw new Error(`Failed to send Wasender message: Connection Timed Out.`);
      }
      ...
      throw new Error(`Failed to send Wasender message: ${status} - ${errorText}.`);
    }
...
function isSimpleGreeting(text: string): boolean {
    ...
    const greetings = [
        "hi", "hello", "hey", "hola", "gm", "good morning", "ge", "good evening",
        "ga", "good afternoon", "hii", "hellloo", "afternoon",
    ];
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (body.event !== "messages.received" || !body.data || !body.data.messages) {
            return NextResponse.json({ status: "ignored", reason: `Not a messages.received event or missing data` }, { status: 200 });
        }
        ...
        if (!recipientId || messageData.key?.fromMe === true || recipientId.endsWith("@g.us")) {
            return NextResponse.json({ status: "ignored", reason: "Ignored sender/recipient" }, { status: 200 });
        }
        
        companyIdentifier = extractCompanyIdentifier(body);

        if (!companyIdentifier) {
            return NextResponse.json({ status: "ignored", reason: "Non-text or empty message type" }, { status: 200 });
        }
...
What company would you like to verify?
            ...
            return NextResponse.json({ status: "success", message: "Welcome message sent" }, { status: 200 });
        }
...
Please double-check the spelling or try using the full **Registration Number**.
        ...
        const genericErrorMessage = "A system error occurred while checking the company. Please try again shortly.";
        if (recipientId) {
             try {
